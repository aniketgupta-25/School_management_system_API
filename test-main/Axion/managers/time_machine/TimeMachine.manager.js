module.exports = class TimeMachine { 
    ...
    async mixTopics({darbId, threshold = 0.15}) {
        try {
            if (!darbId || typeof darbId !== 'string') {
                throw new Error('Invalid darbId provided');
            }
            ...
            const topics = await this.oyster.call('nav_relation', {
                relation: '_members',
                label: 'topic',
                _id: darbId,  // Fixed: Using the passed darbId instead of hardcoded value
                sort: 'h2l',
                withScores: true,
            });
            ...
            for(const topic of Object.keys(topics)) {
                ...
                if(postKeys.length !== 0) {
                    const trimmedPosts = postKeys.splice(0, Math.ceil(postKeys.length * threshold));
                    await this.oyster.call('update_relations', { 
                        _id: mixTopic, 
                        add: { 
                            _members: trimmedPosts 
                        }
                    });
                }
            }
        } catch (error) {
            console.error('Error in mixTopics:', error);
            throw error;
        }
    }
